...
module text_area(output, font, fontSize) {
...
module caption_plate(caption, font, fontSize,  captionElevation, borderThickness) 
{
        ...
        difference()
        {
            linear_extrude( height = captionElevation )
                offset (delta = borderThickness)
                    text_area(caption, font, fontSize);
            linear_extrude( height = captionElevation )
                text_area(caption, font, fontSize);
        }
    }
    ...
    // write the caption
    linear_extrude(height = captionElevation)
        write_caption(caption, font, fontSize);
